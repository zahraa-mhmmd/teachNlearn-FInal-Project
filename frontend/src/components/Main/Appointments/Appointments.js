...
export const renderUnavailabilities = (teacher, cb) => {
  ...
  if (teacher.teacherInfo && teacher.teacherInfo.availability) {
    selectedTeacherUnavailabilites = teacher.teacherInfo.availability;
  } else {
...
export const convertLunchBreakToRRule = lunchBreak => {
  console.log('dtstart:', convertUTC(lunchBreak.lunchBreakStart));
console.log('until:', convertUTC(moment(lunchBreak.lunchBreakEnd).add(1, 'year').toDate()));
...
const Appointments = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      user.isTeacher &&
      _.has(user.teacherInfo.availability, 'openingTime') &&
      _.has(user.teacherInfo.availability, 'closingTime') &&
  ...
  const handleTeacherAvailabilitySubmit = async () => {
    ...
    // delete unavailabilityObj.teacherInfo.availability.errors;
  ...
  const handleSubmit = async () => {
    ...
    if (
      !studentFormState.startTime ||
      !studentFormState.endTime ||
      !studentFormState.teacher ||
      !studentFormState.sessionDuration
    ) {
      setFlashMessage({
    ...
    if (moment(studentFormState.startTime).isSameOrBefore(moment())) {
      setFlashMessage({
    ...
    if (
      moment(studentFormState.endTime).isSameOrBefore(
        moment(studentFormState.startTime)
      )
    ) {
      setFlashMessage({
        message: `Session booking start time can't be in the past`,
    ...
    try {
      const sessionToBook = {
        startTime: moment(studentFormState.startTime).format(
          'YYYY-MM-DD hh:mm a'
        ),
        endTime: moment(studentFormState.endTime).format('YYYY-MM-DD hh:mm a'),
      };
  ...
  const handleSessionDuration = (e, duration) => {
    if (e.target.value === duration) {
      const endTime = moment(studentFormState.startTime)
      ...
      <div className="appointments-wrapper">
        <section className="calendar-form-wrapper">
          <CalendarForm
            ...
            setStudentFormState={setStudentFormState}
