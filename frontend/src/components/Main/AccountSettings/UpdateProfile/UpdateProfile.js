...
;  const { user, setUser } = useContext(AuthContext);
  ...
  const sanitizeForm = () => {
    ...
    if (formFormRequest.isTeacher === false) {
      deleteUnwantedKeys(formFormRequest, 'courses', '_id');
      deleteUnwantedKeys(formFormRequest, 'education', '_id');
    }
  ...
  const checkEmptyInputFields = key => {
    user.studentInfo[key].forEach(el => {
  ...
  const deleteUnwantedKeys = (obj, key, subKey) => {
    obj.studentInfo[key].forEach(el => {
      if (el[subKey]) {
        delete el[subKey];
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    if (user.studentInfo) {
      checkEmptyInputFields('courses');
      checkEmptyInputFields('education');

      if (!user.studentInfo.academicLevel) {
        setFlashMessage({
  ...
  const handleImageUpload = async e => {
    ...
    if (!uploadedImage || !(uploadedImage instanceof File)) {
      setFlashMessage({
        message: `Please include a valid image file`,
        type: 'error',
        icon: 'alert',
      });
      return;
    ...
    try {
      ...
      if (responseFile.data && responseFile.data.Location) {
        const responseUpdateUser = await updateProfile({
          profileImage: responseFile.data.Location,
        });
        setUser(responseUpdateUser.data);
  ...
  <>
    ...
    {user.studentInfo.education.map((val, i) => {
      return (
        <div key={i}>
          <AuthInput
            name="name"
          ...
          <div className="btn-box update-account">
            {user.studentInfo.education.length !== 1 && (
              <Button
                onClick={() =>
                  handleRemoveClick(
                    'studentInfo',
            ...
            {user.studentInfo.education.length - 1 === i && (
              <Button
                onClick={() =>
                  user.studentInfo.education[i].name !== '' &&
                  user.studentInfo.education[i].department !== '' &&
                  user.studentInfo.education[i].gradeLevel !== '' &&
                  user.studentInfo.education[i].notes !== '' &&
                  handleAddMultiClick(
                    'studentInfo',
                    'education',
                    {
    ...
    {user.studentInfo.courses.map((val, i) => {
      return (
        <div key={i}>
          <AuthInput
            name="courseName"
          ...
          />
          <div className="btn-box update-account">
            {user.studentInfo.courses.length !== 1 && (
              <Button
                onClick={() =>
                  handleRemoveClick(
                    'studentInfo',
            ...
            {user.studentInfo.courses.length - 1 === i && (
              <Button
                onClick={() =>
                  user.studentInfo.courses[i].courseName !== '' &&
                  user.studentInfo.courses[i].courseCode !== '' &&
                  user.studentInfo.courses[i].notes !== '' &&
                  handleAddMultiClick(
                    'studentInfo',
                    'courses',
                    {
    ...
    <AuthSelect
      value={user.studentInfo.academicLevel}
      placeholder="Academic Level"
      icon="graduationCap"
      options={academicLevels}
      onValueChange={e =>
        onNestedValueChange(e, 'studentInfo', 'academicLevel')
      }
    />
  </>
)}
